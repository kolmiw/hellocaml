...
let rec insert (x:'a) (l:'a list) : 'a list =
  begin match l with
    | [] -> [x]
    | h::tl -> if h= x then h::tl else if x<h then x::h::tl else h:: insert(x)(tl)
  end
...
let rec lookup (x:string) (c:ctxt) : int64 =
	...
	| [] -> raise Not_found
	| h::tl -> let (a, b) = h in
			if a = x then b else lookup x tl
	end
...
let rec optimize (e:exp) : exp =
  ...
  | Add(a, Const 0L) -> optimize a
  | Add(Const 0L, a) -> optimize a
  | Add(Neg(a), Neg(b)) -> Neg(Add(optimize a, optimize b))
  | Add(a,b) -> let x = Add( optimize a, optimize b) in if e=x then e else optimize x

  | Mult(Const a, Const b) -> Const (Int64.mul a b)
  | Mult(a,Const 0L) -> Const 0L
  | Mult(Const 0L , a) -> Const 0L
  | Mult(a,Const 1L) -> optimize a
  | Mult(Const 1L, a) -> optimize a
  | Mult(Neg(a),Neg(b)) -> Mult(optimize a, optimize b)
  | Mult(Const a, Neg(b)) -> Mult(Const (Int64.mul a (-1L)), (optimize b))
  | Mult(Neg(b), Const a) -> Mult(Const (Int64.mul a (-1L)), (optimize b))
  | Mult(a, Neg(b)) -> Neg(Mult(a, optimize b))
  | Mult(Neg(b), a) -> Neg(Mult(a, optimize b ))
  | Mult(a,b) -> let x = Mult( optimize a, optimize b) in if e=x then e else optimize x

  | Neg(Const 0L) -> Const 0L
  | Neg(a) -> begin match a with | Neg(b) -> optimize b
                                 | Const b ->Const (Int64.mul b (-1L))
                                 | b -> Neg(optimize b)
                                 end
